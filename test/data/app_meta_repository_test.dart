...
void main() {
  ...
  test('빈 DB → 모든 getter 가 null', () async {
    expect(await meta.getWordsVersion(), isNull);
    expect(await meta.getCharsVersion(), isNull);
    expect(await meta.getWordsSyncedAt(), isNull);
    expect(await meta.getLastSyncError(), isNull);
  });

  test('markWordsSynced 후 readback', () async {
    final v = Version.parse('1.2.3');
    await meta.markWordsSynced(v);
    expect(await meta.getWordsVersion(), v);
  ...
  test('성공 sync 는 last_sync_error 를 비운다', () async {
    ...
    await meta.markWordsSynced(Version.parse('2.0.0'));
    expect(await meta.getLastSyncError(), isNull);
  });

  test('손상된 semver 문자열은 null 반환 (throw 하지 않음)', () async {
    // 내부 raw insert 로 손상 데이터 주입.
    await db.customStatement(
      "INSERT INTO app_meta (key, value) VALUES ('words_version', 'not-semver')",
    );
    expect(await meta.getWordsVersion(), isNull);
  });
}
